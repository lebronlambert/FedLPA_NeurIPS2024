...
from model import *
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData,EMNIST_truncated
...
def load_emnist_data(datadir):
    ...
    mnist_train_ds = EMNIST_truncated(datadir, train=True, download=True, transform=transform)
    mnist_test_ds = EMNIST_truncated(datadir, train=False, download=True, transform=transform)
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if partition == "homo":
        ...
        for j in range(n_parties):
            #print(len(idx_batch[j]))
            np.random.shuffle(idx_batch[j])
            #idx_batch[j] = idx_batch[j][0:int(len(idx_batch[j]) * 0.8)]
            #print(len(idx_batch[j]))
...
def compute_accuracy2(model, dataloader, get_confusion_matrix=False, moon_model=False, device="cpu"):
...
class AddGaussianNoise(object):
    ...
    def __call__(self, tensor):
        if self.net_id is None:
            return tensor + laplace_distribution.sample(tensor.shape)+self.mean #torch.randn(tensor.size()) * self.std + self.mean
        else:
            #tmp = torch.randn(tensor.size())
            tmp=laplace_distribution.sample(tensor.shape)
            ...
            tmp = tmp * filt
            return tensor + tmp+ self.mean # * self.std + self.mean

    def __repr__(self):
        return self.__class__.__name__ + '(mean={0}, std={1})'.format(self.mean, self.std)

def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0):
    if dataset in ('mnist', 'femnist', 'fmnist','emnist' ,'cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar100', 'tinyimagenet'):
        if dataset == 'mnist':
